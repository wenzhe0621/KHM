...
imp.layoutm11 <- function(ImpList,VarNames,VariableDensity=TRUE,newPDF=FALSE,the.title=NULL,newPNG=FALSE,newJPEG=FALSE,newEPS=FALSE,Points=NULL,tol=0.5){
  if(newPDF){
    if(is.null(the.title))
      stop("Require a Title for the PDF")
    else
  ...
  #bV <- breakVec
  #bV[8] <- 3.5
  plot.window(c(0,1),c(0,Cutoff+tol),xaxs="i",yaxs="i")
  bV <- breakVec
  bV[8] <- Cutoff+tol
...
imp.layoutmCHANGE11 <- function(ImpList,VarNames,VariableDensity=TRUE,newPDF=FALSE,the.title=NULL,newPNG=FALSE,newJPEG=FALSE,newEPS=FALSE,Points=NULL){
  if(newPDF){
    if(is.null(the.title))
      stop("Require a Title for the PDF")
    else
